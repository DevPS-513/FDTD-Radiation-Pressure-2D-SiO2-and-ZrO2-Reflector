...
% load power
     ...
     TM_flux=load(['./Figure_Data/AB_' prefix '_sim_case_' sim_case '_dx_'...
step_size '_nm_Sx_flux_vs_t.dat']);

     TM_flux_ref=load(['./Figure_Data/AB_' prefix '_sim_case_' sim_case '_dx_'...
...
% Load TE Power
     ...
     TE_flux=load(['./Figure_Data/AB_' prefix '_sim_case_' sim_case '_dx_'...
step_size '_nm_Sx_flux_vs_t.dat']);

     TE_flux_ref=load(['./Figure_Data/AB_' prefix '_sim_case_' sim_case '_dx_'...
step_size '_nm_Sx_flux_ref_vs_t.dat']);
...
fz_norm_TE=max(MN_fz_avg_TE(:,2));


 % DEFINE NORMALIZATION   

    dz=AB_fz_avg_TM(2,1)-AB_fz_avg_TM(1,1);
    source_location=1725*micrometers;
    source_location_n=round(source_location/dz);
    grating_location=2.01*micrometers;
    f_AB_pk_TM=max(AB_fz_avg_TM(source_location_n:end,2));
    f_AB_pk_TE=max(AB_fz_avg_TE(source_location_n:end,2));
...
subplot(3,1,3)
er_fac=1./max(AB_fz_avg_TM(:,2))*max(EL_tau_peak_TM(:,2));
