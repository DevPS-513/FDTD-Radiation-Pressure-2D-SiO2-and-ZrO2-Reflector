...
addpath TE_EM_functions        % folder with sub-functions
...
% Temporal
    ...
    fig_refresh=T*10;                    % 10 figures per cycle
...
% SOURCE GEOMETRY
    ...
    source_initial_phase=90*degrees;
...
% GRID SIZE
   ...
   % avg_time=2*1.5*x_size/(c/1.88);                          % start taking average 9 cycles before simulation is finished   
...
% Initialize Fields and Current matricies
 ...
 % Spatial average quantities
    ...
    Ez_at_Bx=zeros(Nx,Ny);           % Hz placed at Ex
    Ez_at_By=zeros(Nx,Ny);           % Hz placed at Ey
    Dz_at_Bx=zeros(Nx,Ny);           % Bz placed at Ex
    Dz_at_By=zeros(Nx,Ny);           % Bc placed at Ey
...
%% COEFFICIENTS
    ...
    E2=1./(eps_o.*er./dt+sigma_e./2);
    
    H1=(-mu_o.*mr./dt+sigma_m./2)./(-mu_o.*mr./dt-sigma_m./2);        % PML only
    H2=1./(-mu_o.*mr./dt-sigma_m./2)/dx;
...
% PLOT HZ FIELD
    figure(2)                           
    h_Hz=surf(x*1e6,y*1e6,Ez');                               % surf the Hz field
    hold on
...
% simple Ex 
 % Ex(2:Nx,2:Ny)=Ex(2:Nx,2:Ny)+E1(2:Nx,2:Ny).*(Hz(2:Nx,2:Ny)-Hz(2:Nx,1:Ny-1)); % lossless
  ...
  %   H1=(-mu_o.*mr./dt+sigma_m./2)./(-mu_o.*mr./dt-sigma_m./2);        % PML only
    ...
    Hx(2:Nx,2:Ny)=H1(2:Nx,2:Ny).*Hx(2:Nx,2:Ny) + H2(2:Nx,2:Ny).*(Ez(2:Nx,2:Ny)-Ez(2:Nx,1:Ny-1)); %PML only
    Bx=mu_o.*mr.*Hx;
...
% simplest form % Ey(2:Nx,2:Ny)=Ey(2:Nx,2:Ny)-E1(2:Nx,2:Ny).*(Hz(2:Nx,2:Ny)-Hz(1:Nx-1,2:Ny));
    ...
    Hy(2:Nx,2:Ny)=H1(2:Nx,2:Ny).*Hy(2:Nx,2:Ny) -H2(2:Nx,2:Ny).*(Ez(2:Nx,2:Ny)-Ez(1:Nx-1,2:Ny));
    By=mu_o.*mr.*Hy;  
   
 for gi=1:length(gauss_y)  
     Hy(source1_x,gi)=source_direction*gauss_t(n).*1*gauss_y(gi).*(.5*a/eta_o).*sin(2*pi*f*(t(n))-source_initial_phase)+Hy(source1_x,gi); % Y- Guassian     
 end
...
Tx=calculate_Tx_MN_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gx_n_prev(i,j)=Gx(i,j);
Gx(i,j)=-1.*Ez_at_By(i,j).*Hy(i,j)./c^2;  
Sx(i,j)=Gx(i,j).*c^2;

% y-calcualtions
Ez_at_Bx(i,j)=.25*(Ez(i,j)+Ez(i,j-1)+Ez_n_prev(i,j)+Ez_n_prev(i,j-1));
[Ty,t1,t2,t3,t4]=calculate_Ty_MN_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gy_n_prev(i,j)=Gy(i,j);
Gy(i,j)=Ez_at_Bx(i,j).*Hx(i,j)./c^2;
end
...
Tx=calculate_Tx_MN_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gx_n_prev(i,j)=Gx(i,j);
Gx(i,j)=-1.*1.*Dz_at_By(i,j).*By(i,j);
Sx(i,j)=Gx(i,j).*c^2;
% y-calculations
% y-calcualtions
Dz_at_Bx(i,j)=.25*(Dz(i,j)+Dz(i,j-1)+Dz_n_prev(i,j)+Dz_n_prev(i,j-1));
Ty=calculate_Ty_MN_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gy_n_prev(i,j)=Gy(i,j);
Gy(i,j)=Dz_at_Bx(i,j).*Bx(i,j);
...
% x-calculations
Ez_at_By(i,j)=.25*(Ez(i,j)+Ez(i-1,j)+Ez_n_prev(i,j)+Ez_n_prev(i-1,j));
Tx=calculate_Tx_Chu_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gx_n_prev(i,j)=Gx(i,j);
Gx(i,j)=-1.*1.*Ez_at_By(i,j).*Hy(i,j)./c^2;  
Sx(i,j)=Gx(i,j).*c^2;

% y-calcualtions
Ez_at_Bx(i,j)=.25*(Ez(i,j)+Ez(i,j-1)+Ez_n_prev(i,j)+Ez_n_prev(i,j-1));
Ty=calculate_Ty_Chu_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gy_n_prev(i,j)=Gy(i,j);
Gy(i,j)=Ez_at_Bx(i,j).*Hx(i,j)./c^2;

%W(i_W,j)=W(i_W,j)-(c.^2.*(dt/dx)*(Gx(i_W+1,j)-Gx(i_W,j))-c.^2*(dt/dy).*(Gy(i_W,j+1)-Gy(i_W,j)));
...
% x-calculations
Ez_at_By(i,j)=.25*(Ez(i,j)+Ez(i-1,j)+Ez_n_prev(i,j)+Ez_n_prev(i-1,j));
Tx=calculate_Tx_EL_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gx_n_prev(i,j)=Gx(i,j);
Gx(i,j)=-1.*1.*Ez_at_By(i,j).*Hy(i,j)./c^2;  
Sx(i,j)=Gx(i,j).*c^2;
...
% y-calcualtions
Ez_at_Bx(i,j)=.25*(Ez(i,j)+Ez(i,j-1)+Ez_n_prev(i,j)+Ez_n_prev(i,j-1));
Ty=calculate_Ty_EL_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gy_n_prev(i,j)=Gy(i,j);
Gy(i,j)=Ez_at_Bx(i,j).*Hx(i,j)./c^2;
...
Ez_at_By(i,j)=.25*(Ez(i,j)+Ez(i-1,j)+Ez_n_prev(i,j)+Ez_n_prev(i-1,j));
Tx=calculate_Tx_AMP_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gx_n_prev(i,j)=Gx(i,j);
Gx(i,j)=-1.*eps_o.*Ez_at_By(i,j).*By(i,j);
Sx(i,j)=Gx(i,j).*c^2;

% y-calcualtions
Ez_at_Bx(i,j)=.25*(Ez(i,j)+Ez(i,j-1)+Ez_n_prev(i,j)+Ez_n_prev(i,j-1));
Ty=calculate_Ty_AMP_TE(i,j, Bx,By,Hx,Bx_n_prev,By_n_prev,Hx_n_prev,Hy_n_prev,Hy,Ez,Dz,dx,dy );
Gy_n_prev(i,j)=Gy(i,j);
Gy(i,j)=eps_o.*Ez_at_Bx(i,j).*Bx(i,j);
end



    Sx_flux_ref(1,n)=sum(Sx(refi,refj))*dx;
...
%% Update Ez (n+1/2)
    H_term=1*(1/dx).*(Hy(2:Nx,1:Ny-1)-Hy(1:Nx-1,1:Ny-1))...
            -(1/dy).*(Hx(1:Nx-1,2:Ny)-Hx(1:Nx-1,1:Ny-1));
         
 Ez(1:Nx-1,1:Ny-1)=E1(1:Nx-1,1:Ny-1).*Ez(1:Nx-1,1:Ny-1)...
    +E2(1:Nx-1,1:Ny-1).*H_term; % lossless
 ...
 %% FDTD Ez Source
for gi=1:length(gauss_y)  
Ez(source1_x,gi)=-1*gauss_t(n).*gauss_y(gi).*(a/(2)).*sin(2*pi*f*(t(n))-source_initial_phase)+Ez(source1_x,gi);  % X- Guassian %         
end          
...
%% Pulse Calculations
    ...
    pulse_momentum_x(1,n)=sum(sum(Gx(i,j)))*dx*dy;
    

    
    pulse_momentum_y(1,n)=sum(sum(Gy(i,j)))*dx*dy;
...
g_mech_x(i,j)=g_mech_x(i,j)-dt.*Tx(i,j)-1*(Gx(i,j)-Gx_n_prev(i,j));
g_mech_y(i,j)=g_mech_y(i,j)-dt.*Ty(i,j)-1*(Gy(i,j)-Gy_n_prev(i,j));    
